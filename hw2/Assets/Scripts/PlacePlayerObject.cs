...
public class PlacePlayerObject : MonoBehaviour
{
        ...
        if (!placed)
        {
            if (Input.GetMouseButtonDown(0))
            {
                    ...
                    touchPosition, ref hits, TrackableType.PlaneWithinPolygon))
                {
                    ...
                    // Now we will activate our player object and place it at the
                    // chosen location.
                    // boardPosition = labyrinthBoard.whereAreYou();
                    rb.transform.position = hitPosition;
                    placed = true;
                    // After we have placed the player object we will disable the
    ...
    // If the user places the player object at an undesirable location we 
    // would like to allow the user to move the player object to a new location.
    public void AllowMovePlayerObject()
    {
        placed = false;
    ...
    // player object has been places so we will add an accessor to this.
    public bool Placed()
    {
        return placed;
    }
